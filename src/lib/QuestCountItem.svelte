...
<div class="col-xxxl-2 col-xxl-3 col-xl-4 col-lg-4 col-md-6 mb-4">
  <div class="card card-quest h-100">
    <div style="position: relative;">
      <img src={getImageUrl(count) + ""} class="card-img-top" alt={count.name} />
      <div class="floating-text count-container">{count.count}</div>
      <div class="floating-text count-location">
        {@html highlightMatch(count.location, query)}
      </div>
      <div class="floating-text count-trader">
        {@html highlightMatch(count.trader, query)}
      </div>
    </div>
    <div class="card-body">
      <h5 class="card-title"><QuestLink quest={count} {query} /></h5>
      ...
      <button
        ...
        data-bs-content={count.description}
      >
    ...
    <div class="card-body">
      ...
      {#each sortedUSers as user}
        <User {user} {query} tasks={count.userTasks.find(d => d.user === user)?.tasks || []} />
      {/each}
    </div>
  </div>
</div>
